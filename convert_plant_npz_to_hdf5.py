"""NPZ → sequential HDF5 for GenomicHDF5Dataset and hidden-state caching.

Input layout (from prepare_data.py):
  <dataset_root>/<species>/output_labels/<Chr>.npz   # falls back to species root

Output HDF5 (smORFeus convention):
  [seq_id][chunk_id]/sequence       S1 bytes  (CHUNK_SIZE,)
  [seq_id][chunk_id]/frame_targets  uint8     (CHUNK_SIZE, 7)
  [seq_id][chunk_id]/target         uint8     (CHUNK_SIZE, 2)  # legacy stub

  seq_id   = "{species}_{chrom}"
  chunk_id = "{seq_id}_chunk_{N:05d}"   (matches r".+_chunk_\\d{5}")

Label order matches prepare_data.LABELS.
"""
...
def _sanitise_name(name: str) -> str:
    """Replace HDF5-illegal characters with underscore."""
    return re.sub(r"[^A-Za-z0-9_.\-]", "_", name)


def _load_npz(path: str) -> tuple[str, np.ndarray]:
    """Returns (sequence_str, labels(L,7))."""
...
def _pad_to_chunk(seq_slice: str, label_slice: np.ndarray) -> tuple[str, np.ndarray]:
    """Right-pad a partial final chunk with Ns and zero labels."""
...
def _write_chunk(
    ...
    # Legacy 2-channel stub for older code paths
