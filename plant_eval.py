"""Evaluation callback and standalone evaluation for plant genome annotation.

This module provides PlantEvalCallback (a PyTorch Lightning Callback) that
collects per-label probabilities during the test epoch and produces
publication-ready figures, and evaluate_from_checkpoint() for running
standalone evaluation against a saved checkpoint.

Figures produced per label:
    - ROC curves with AUC
    - Precision-Recall curves with Average Precision
    - Row-normalised confusion matrices
    - Multi-track genomic overlay panels (predicted vs. true annotation tracks)
"""
...
def _extract_runs(x: np.ndarray) -> List[tuple]:
    """Return (start, end) pairs for each contiguous run of 1s in x."""
    runs, s = [], None
...
class PlantEvalCallback(Callback):
    """Collects per-label probabilities over the test epoch and saves figures."""
    ...
    def _reset(self):
        """Clear accumulated per-epoch probability and target buffers."""
