"""Collator for plant genome annotation batches.

Converts lists of raw dataset items (sequence strings + label arrays) into
padded tensors ready for model input. The number of labels is inferred
dynamically from the first item in each batch, making this collator
compatible with any number of annotation channels.

Classes exposed:
    PlantCollator -- Callable collator producing padded batches with frame
                     phase, core-region masking, and optional RC augmentation.
"""
...
class PlantCollator:
    """Collate dataset items into padded batches with frame phase and core-region masking."""
    ...
    def __init__(
        ...
        # Build a 256-element ASCII lookup table for O(1) vectorised tokenisation
        self._lookup = np.full(256, self.pad_token_id, dtype=np.int64)
        for char, idx in self.token_map.items():
            self._lookup[ord(char)] = idx
            self._lookup[ord(char.lower())] = idx  # handle lowercase input
    ...
    def __call__(self, batch):
        ...
        # Infer num_labels from the first valid 2-D target tensor
        num_labels = None
        for item in batch:
